...
template<typename T> class result {
   public:
    ...
    template<class... Args>
    constexpr explicit result(Args&&... args) : my_result(std::in_place_type<T>, std::forward<Args>(args)...) {}
    ...
    constexpr const T& operator*() const& {
        return std::get<value_index>(my_result);
    }
    ...
    constexpr bool has_value() const noexcept {
        return my_result.index() == value_index;
    }
   ...
   private:
    ...
    static constexpr size_t error_index = 0;
